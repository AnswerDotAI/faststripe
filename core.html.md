...
<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L25"
...
<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L47"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L32"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L59"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L52"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L90"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L106"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L116"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L121"
target="_blank" style="float:right; font-size:smaller">source</a>
...
The Stripe CLI forwards real webhook events to a local route, so a
notebook or a test receives the same signed requests a deployed app
would.
[`stripe_listen`](https://AnswerDotAI.github.io/faststripe/core.html#stripe_listen)
runs `stripe listen --forward-to url` for you: it waits until the CLI
reports ready, keeps the CLI’s output in `log`, and returns a
[`StripeListener`](https://AnswerDotAI.github.io/faststripe/core.html#stripelistener)
whose `secret` is the signing secret the route must verify with and
whose `close` stops the process. A listener already forwarding to that
URL, from another kernel say, is reused rather than duplicated; its
secret is then unknown, so `secret` is `None` and `close` leaves it
running.

[`StripeListener`](https://AnswerDotAI.github.io/faststripe/core.html#stripelistener)
is the handle
[`stripe_listen`](https://AnswerDotAI.github.io/faststripe/core.html#stripe_listen)
returns: the CLI process, the signing secret parsed from its ready line,
and a `log` of what it has printed since.

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L137"
target="_blank" style="float:right; font-size:smaller">source</a>
...
``` python
def StripeListener(
    url, proc:NoneType=None, secret:NoneType=None
):
```

*A `stripe listen` process forwarding webhooks to `url`; `close` stops
it when this object started it*

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L153"
target="_blank" style="float:right; font-size:smaller">source</a>
...
async def stripe_listen(
    ...
    api_key:NoneType=None, # Stripe secret key for the CLI; `STRIPE_SECRET_KEY` if None, and the CLI's own login when that is unset too
    timeout:int=30, # Seconds to wait for the CLI to report ready
):
```

*Start `stripe listen --forward-to url` unless one is already running,
and wait for the CLI to report ready*

[`stripe_trigger`](https://AnswerDotAI.github.io/faststripe/core.html#stripe_trigger)
is the sending side: it asks the CLI to fabricate an event of a given
type, with `overrides` setting fixture fields such as the amount or the
customer, and returns once the CLI has sent it.

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L182"
target="_blank" style="float:right; font-size:smaller">source</a>
...
async def stripe_trigger(
    ...
    overrides:NoneType=None, # Fixture fields to set, e.g. `{'payment_intent:amount': 1000}`
    api_key:NoneType=None, # Stripe secret key for the CLI; `STRIPE_SECRET_KEY` if None, and the CLI's own login when that is unset too
):
```

*Run `stripe trigger event`, returning the CLI’s output once the event
has been sent*

``` python
listener = await stripe_listen('http://localhost:8000/webhook')
sapi.webhook_key = listener.secret
listener
```
...
<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L197"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/faststripe/blob/main/faststripe/core.py#L208"
target="_blank" style="float:right; font-size:smaller">source</a>
...
For events that are awkward to cause for real, a fabricated one goes
through the same route:

``` python
print(await stripe_trigger('payment_intent.succeeded', {'payment_intent:amount': 1_000}))
```

Stopping the listener when done leaves nothing running behind the
notebook:
