...
# %% auto #0
__all__ = ['stripe_api_url', 'names', 'op2nm', 'StripeError', 'StripeSignatureError', 'camel', 'StripeObject', 'stripe_obj',
           'StripeApi', 'StripeApiAsync', 'paged', 'pages', 'verify_webhook']

# %% ../nbs/01_core.ipynb #7c62c2fe
from fastcore.all import *
from .endpoints import eps
from .spec import docs_url, obj_typs
from inspect import Parameter, Signature, signature
from urllib.parse import quote

import json,hmac,hashlib,time,re,httpx

# %% ../nbs/01_core.ipynb #18d69832
stripe_api_url = 'https://api.stripe.com'

# %% ../nbs/01_core.ipynb #c4959d0d
...
# %% ../nbs/01_core.ipynb #bcc8fa1e
class _OAPIObj: pass

# %% ../nbs/01_core.ipynb #c27e5cf7
...
def stripe_obj(x):
    if isinstance(x, list): return L(x).map(stripe_obj)
    if not isinstance(x, dict): return x
    o = x.get('object')
    d = AttrDict({k:stripe_obj(v) for k,v in x.items()})
...
# %% ../nbs/01_core.ipynb #ff0b6058
class StripeApi:
    def __init__(self, api_key=None, webhook_key=None, publishable_key=None, service_name='faststripe', base_url=stripe_api_url):
        ...
        self.service_name,self.base_url = service_name, base_url.strip('/')
        self.hdrs = {'Authorization': f'Bearer {self.api_key}'}
        verbs = L(eps).map(lambda x: _OAPIVerb(**x, url=self.base_url, hdrs=self.hdrs, client=self))
...
class StripeApiAsync(StripeApi):
    async def __call__(self, path:str, verb:str=None, headers:dict=None, route:dict=None, query:dict=None, data=None):
        ...
        async with httpx.AsyncClient() as cli:
            resp = (await cli.request(verb, f"{self.base_url}/{path}", headers=headers, params=query, data=data))
            try: resp.raise_for_status()
            except httpx.HTTPStatusError as e: raise StripeError(e) from None
        self.recv_hdrs = dict(resp.headers)
        return stripe_obj(resp.json())
...
# %% ../nbs/01_core.ipynb #432001ef
# @patch
# def priced_product(self:StripeApi, product_name, amount_cents, currency='usd', recurring=None, description=None, metadata=None):
...
#     if metadata: prod_params['metadata'] = metadata
...
#     _, price = self.priced_product(product_name, amount_cents, currency, metadata=metadata)
...
async def parse_webhook(self:StripeApi, req):
    ...
    return stripe_obj(json.loads(evt))
